    ...
    for filename in namelist:
        ...
        for hdu_num in range(1,len(data)):
            ...
            if ra_min <= data[hdu_num].header['RA'] <= ra_max and dec_min <= data[hdu_num].header['DEC'] <= dec_max:
                ...
                #z.extend(z_add)
                z_add=data[hdu_num].header['Z']
                z.append(z_add)
                #z[i]=data[hdu_num].header['Z']
                #delta.extend(Table(data[hdu_num].data)['DELTA'])
                ra.append(data[hdu_num].header['RA'])
                #ra[i]=data[hdu_num].header['RA']
                dec.append(data[hdu_num].header['DEC'])
                #dec[i]=data[hdu_num].header['DEC']
                i=i+1 
...
data = fits.open('delta-100.fits')
print(data[1].header['Z'])
...
prepared.write('quasars.fits', format='fits', overwrite=True)
