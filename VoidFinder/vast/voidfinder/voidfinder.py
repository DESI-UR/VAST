...
def filter_data(data_table,
    ...
    if dist_limits is not None:
        ...
        if dist_metric == 'comoving':
            ...
            distance_boolean = np.logical_and(data_table['Rgal'] >= dist_limits[0], 
                                              data_table['Rgal'] <= dist_limits[1])
        else:

            H0 = 100*h

            distance_boolean = np.logical_and(c*data_table['redshift']/H0 >= dist_limits[0],
                                              c*data_table['redshift']/H0 <= dist_limits[1])

        data_table = data_table[distance_boolean]


    # Convert galaxy coordinates to Cartesian
    coords_xyz = ra_dec_to_xyz(data_table, dist_metric, h)
    ############################################################################
...
def ra_dec_to_xyz(data_table,
    ...
    if distance_metric == 'comoving':
        ...
        r_data = c*data_table['redshift'].data/(100*h)
